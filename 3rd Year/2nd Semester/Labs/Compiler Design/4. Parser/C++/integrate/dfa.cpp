/**
001210501027 : Afif Ahmed
001210501029 : Indranil Guha
*/
#include <iostream>
#include <cstdlib>
#include <vector>
...
class Dfa
{
	...
	bool check_expression(string x)
	{
		int s=0;
		for(int i=0;i<x.size();i++)
		{
			s=V[s].getTransition(x[i]);
			if(s==-1)
				return false;	
		}
		return V[s].exists(leaves.size()-1);
	}
	...
	void createDfa()
	{
		...
		// cout<<regex<<endl;
		// return;
		s=string(regex);//gets input converted to postfix
...
// int main()
// {
// 	Dfa *id_regex=new Dfa(string("ab+"));
// 	id_regex->createDfa();

// 	Dfa *num_regex=new Dfa(string("ab*"));
// 	num_regex->createDfa();
// }