...
int main()
{
    ...
    while(1)
    {
	    ...
	    int n=strlen(s),i=0,no_of_ack=0;
	    while(i<n)
	    {
    		int count=0;
    		if (i%5==0)
    		{
    			memset(retrans_frames, 0, size*sizeof(retrans_frames[0]));
    		}
	    	if(resend_flag==0)
	    	{
	        ...
	        while(c<count)
	        {
	        	...
	        	while(time_out--)
	        	{
			        ...
			        {
			            ack_flag = 1;
			            retrans_frames[(acknowledgement.seq_num)%size]=1;
			    	    break;
			        }
			        sleep(1);
		    	}
